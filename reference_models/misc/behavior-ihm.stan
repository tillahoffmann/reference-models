...
parameters {
    ...
    matrix[K - 1, N_month] z_month;      // matrix of monthly standardized random effects
    vector<lower=0>[K - 1] sigma_month;   // stddev of monthly random effects
    cholesky_factor_corr[K - 1] L_Rho_month;         // correlation matrix of monthly random effects
}

transformed parameters {
    matrix[N_id, K - 1] v_id = (diag_pre_multiply(sigma_id, L_Rho_id) * z_id)';
    matrix[N_house, K - 1] v_house = (diag_pre_multiply(sigma_house,L_Rho_house) * z_house)';
    matrix[N_month, K - 1] v_month = (diag_pre_multiply(sigma_month,L_Rho_month) * z_month)';
}
...
generated quantities{
    ...
    matrix[K - 1, K - 1] Rho_house = L_Rho_house * L_Rho_house';
    matrix[K - 1, K - 1] Rho_month = L_Rho_month * L_Rho_month';
